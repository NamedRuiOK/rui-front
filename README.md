...
2. `src/api/auth.js` 向同源接口发送 JSON POST 请求：
...
4. 接口返回 `code === 200` 且包含 `data.accessToken` 时，Token 通过 `localStorage` 保存为：

   ```text
   accessToken
   ```

5. 页面显示登录成功状态。切换账户时只清空当前密码和页面状态，不会自动删除已保存 Token。
...
- 登录接口默认运行在 `localhost:6000`。
- 开发环境通过 Vue CLI 的 `/api` 代理访问后端，避免浏览器跨域、混合内容和本地网络访问策略影响登录请求。
- 可通过 `VUE_APP_DEV_API_TARGET` 修改开发代理目标，例如 `http://127.0.0.1:6000`。
- 可通过 `VUE_APP_API_BASE_URL` 修改浏览器请求前缀；生产环境默认要求部署服务器将 `/api` 反向代理到后端。
- 开发环境登录时，浏览器 Console 会打印可在 Windows PowerShell 中执行的 `curl.exe` 命令，以及请求来源和响应状态。
- 调试 cURL 包含登录密码，仅用于本地排查；生产构建不会输出该日志。
